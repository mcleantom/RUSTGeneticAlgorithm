...
impl Individual {
    ...
    pub fn cross_over(&self, other: &Individual, cities: &[City]) -> (Self, Self) {
        ...
        let daugher_dna = crossover_dna(&self.dna, &other.dna, start, end);
        let son_dna = crossover_dna(&other.dna, &self.dna, start, end);
        let daughter = Individual::new(daugher_dna, cities);
...
fn fitness(dna: &[usize], cities: &[City]) -> f64 {
    let d = dna.windows(2).fold(MIN_POSITIVE, |acc, w| {
        acc + cities[w[0]].distance_squared(&cities[w[1]])
    });
    1.0 / d
}
