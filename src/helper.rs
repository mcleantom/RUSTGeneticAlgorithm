...
pub fn parse_specs(contents: &str) -> Result<(usize, usize, usize, usize, f64, f64), String> {
    ...
    let debug_level: usize = v[0].parse().map_err(|err| {
        format!(
            "debug_level = {} can't be parsed as integer.\nerror: {}\n",
            v[0], err
        )
    })?;
    ...
    let skip: usize = v[1].parse().map_err(|err| {
        format!(
            "skip = {} can't be parsed as integer.\nerror: {}\n",
            v[1], err
        )
    })?;
    ...
    let iterations: usize = v[2].parse().map_err(|err| {
        format!(
            "iterations = {} can't be parsed as integer.\nerror: {}\n",
            v[2], err
        )
    })?;
    ...
    let population_size: usize = v[3].parse().map_err(|err| {
        format!(
            "population_size = {} can't be parsed as integer.\nerror: {}\n",
            v[3], err
        )
    })?;
    ...
    let crossover_probability: f64 = v[4].parse().map_err(|err| {
        format!(
            "crossover_probability = {} can't be parsed as a float.\nerror: {}\n",
            v[4], err
        )
    })?;
    ...
    let mutation_probability: f64 = v[5].parse().map_err(|err| {
        format!(
            "mutation_probability = {} can't be parsed as a float.\nerror: {}\n",
            v[5], err
        )
    })?;
